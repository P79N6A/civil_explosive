...
require(['garenjs/utils','garenjs/htmlui','garenjs/easyui'],
	...
	function resetPanelSize(target){
		...
		myobj.find("> .layout_center").outerHeight(centerSize.height)
					.outerWidth(centerSize.width);
		opts.onResize && opts.onResize.call(target,width,height);//触发事件
	}
	//生成组件
	...
	function setPanelSize(target,params){
		...
		if(panelUI){
			if(params.width != null) {
				...
				if(params.width == 0) panelObj.hide();
				else panelObj.show();
			}
			if(params.height != null) {
				...
				if(params.height == 0) panelObj.hide();
				else panelObj.show();
			}
