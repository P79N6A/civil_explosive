...
require(['garenjs/utils','garenjs/htmlui','garenjs/easyui'],
	function(utils,garenUI,easyUI){
		...
		function init(target){
			...
			//创建表格行列
			var tbody = createTBody(opts);
			//创建单元格合并
			createTdSpan(opts);
			//创建表头
			var thead = createHeader(opts);
			var ui = [{
				eName:'thead',
				elements:thead
			},{
			...
			myobj.createHtml(ui);
			var tdNodes = myobj.find('td');
			//遍历设置样式
			tdNodes.each(function(){
				var r = $(this).parent().prop('rowIndex');
				var c = $(this).prop('cellIndex');
				if(opts.onTdStyle) opts.onTdStyle.call(this,r,c);
			});
			//绑定事件
			tdNodes.click(function(){
				var r = $(this).parent().prop('rowIndex');
				var c = $(this).prop('cellIndex');
				if(opts.onTdClick) opts.onTdClick.call(this,r,c,opts);
			});
			//绘制子元素
			var cells = opts.cells;
			$.each(cells,function(i,row){
				$.each(row,function(j,cell){
					var node = myobj.find("> tbody > tr:eq("+i+") td:eq("+j+")");
		...
		function createHeader(opts){
			var theadCols = opts.columns;
			if($.isArray(theadCols) == false 
					|| theadCols.length == 0) return null;
			var theadHeight = opts.theadHeight || 30;
